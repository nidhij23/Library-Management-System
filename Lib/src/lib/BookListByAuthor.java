...
public class BookListByAuthor extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae)
    {
        if(ae.getSource()==b1)
        {
            ...
            if(p.equals(""))
        {
             JOptionPane.showMessageDialog(this,"Please fill in the Author Name");
        }
 else
            {
    ...
    try
        {
        pst=c.prepareStatement("Select * from bookdatabase where Author=?");
        pst.setString(1,p);
        rs=pst.executeQuery();
        int r=0;
    while(rs.next())
    ...
    {
        BookListByAuthor b=new BookListByAuthor();
    }
}
