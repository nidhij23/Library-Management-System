...
public class BookListByName extends JFrame implements ActionListener{
...
if(ae.getSource()==b2)
        ...
        if(ae.getSource()==b1)
        {
            String p=t1.getText();
                   if(p.equals(""))
        {
             JOptionPane.showMessageDialog(this,"Please fill in the Book Name");
        }
 else
                   {
    ...
    try
        {
        pst=c.prepareStatement("Select * from bookdatabase where BName=?");
        pst.setString(1,p);
        rs=pst.executeQuery();
    ...
    {
        BookListByName b=new BookListByName();
    }
}
